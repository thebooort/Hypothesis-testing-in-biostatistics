...
En general, dentro de los test de hipótesis, vamos a llamar \textbf{hipótesis nula} o $\textbf{H}_\textbf{0}$ a la hipótesis que se asume sobre nuestra distribución y la cual se pretende contrastar. Frente a ella, nos encontramos la \textbf{hipótesis alternativa} o $\textbf{H}_\textbf{1}$, que agrupa aquellas distribuciones que no poseen $H_0$ como cualidad.
...
\begin{enumerate}
	\item Fijar una cota para cometer error de tipo I , $\alpha$, llamada nivel de significación.
...
Una vez tenemos los conceptos básicos en cuanto al contraste de hipótesis, vamos a ofrecer un pequeño recorrido por los test paramétricos que están relacionados con las distribuciones normales.
 ...
 \subsubsection{Contraste t de una muestra para la media de una distribución normal con varianza desconocida (Media alternativa <media nula)  \todo{y este título wtf?}}
...
Para lo cual deberemos usar una tabla de la distribución $\chi^2$ o un ordenador.
 ...
 \subsubsection*{Caso práctico en bioestadística}
(De \cite{rosner2015fundamentals})
\todo{ Trasladar el que empieza en el ejemplo 6.39 y sigue por 7.46 y 7.47}
...
Dadas dos poblaciones de tamaños $n_1$ y $n_2$ distribuidas mediante una normal que poseen la misma varianza, suponemos que queremos contrastar la hipótesis:
\begin{itemize}
	\item $H_0:\mu_1=\mu_2$
	\item $H_1:\mu_1\neq \mu_2$
\end{itemize}
...
En este caso el estadístico sigue una distribución $F$ de Snedecor y, el test, viene dado por el siguiente planteamiento:


Dadas dos poblaciones de tamaños son  $n_1$ y $n_2$ distribuidas mediante una normal que poseen varianzas y medias distintas. Suponemos que queremos contrastar la hipótesis:
\begin{itemize}
...
Un vez obtenido el valor, concluimos mediante:
\begin{itemize}
	\item Si $F>F_{n_1-1,n_2-1.1-\alpha/2}$ ó $F<F_{n_1-1,n_2-1.1-\alpha/2}$, rechazamos $H_0$
...
\todo{Se pueden incluir gráficas así si queréis}
 \subsubsection*{Caso práctico en bioestadística}\todo{Trasladar el que empieza en el ejemplo 8.13-8.17} 
 (De \cite{rosner2015fundamentals}) 
