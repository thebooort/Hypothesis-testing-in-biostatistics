...
\begin{itemize}
    \item si se lleva a cabo dividiendo el espacio muestral $\Omega$ en dos regiones distintas: \todo{qué espacio muestral??}
	\begin{itemize}
...
\subsubsection{Valor $p$}
    ...
    Sin embargo, podría interesarnos saber como de seguro es el rechazo o aceptación obtenidos, es decir, cuantificar significación estadística del test ($\alpha$ sólo nos da una cota).
...
\section{Test paramétricos para una población normal}
     ...
     Dentro de los distintos tipos de tests estadísticos, los \textbf{tests paramétricos} son aquellos en los que se asume cierta distribución para la muestra y se contrasta el valor de sus parámetros.
     Concretamente, nosotros nos centraremos en el caso en el que la muestra sigue una distribución normal.
     
\subsection{Introducción}
En el marco de los problemas paramétricos, el criterio descrito para la selección de test puede formularse como:\todo{¿por qué tanta cursiva?}
...
\todo{qué significa el tamaño del test y por qué es deseable que coincida con el nivel de significación?}
...
A este tipo de test  en el que el valor del parámetro en la hipótesis alternativa puede que ser mayor o menor, pero no ambos, se le llama \textbf{contraste unilateral}.
 ...
 \todo{qué es el p-valor?? Explicar antes}
 ...
 $$p_{valor}=P[t_{n-1}> t]$$
 \todo{hay que explicar qué significa el valor p}
...
%(De \cite{rosner2015fundamentals}) Trasladar el que empieza en el ejemplo 6.39 y sigue por 7.46 y 7.47
    ...
    \todo{en el libro se amplia esto con cálculo del valor p e intervalo de confianza}
...
Dadas $m$ muestras de tamaños $n_1, n_2,...,n_m$ nos preguntas: ¿vienen éstas de la misma distribución poblacional?
